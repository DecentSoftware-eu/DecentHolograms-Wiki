---
title: 'config.yml'
description: 'The config.yml, its structure and options'
---

The `config.yml` contains various settings that can be changed to your liking.

/// details | Default config.yml
...
/// note | Visibility is also influenced by the Client's and Server's entity render distance.
///
...
Contains the default heights for the individual [Line Types](../features/line-content.md)

#### `text`

Sets the height for [Text Lines](../features/line-content.md#text-line).

#### `icon`

Sets the height for [`#ICON` Lines](../features/line-content.md#icon).

#### `head`

Sets the height for [`#HEAD` Lines](../features/line-content.md#head-smallhead).

#### `smallhead`

Sets the height for [`#SMALLHEAD` Lines](../features/line-content.md#head-smallhead).
...
Enables/Disables the parsing of Placeholders (both own and PlaceholderAPI ones) inside [Animations](../features/animations/index.md).  
...
Contains a set of keys and values, with the keys being replaced by the value in any [Text Line](../features/line-content.md#text-line).

As an example, `#!yaml '[x]': '█'` replaces any appearance of `[x]` with `█`.  
